...
**🔧 Devam eden — onboarding tek-soru-tek-ekran restructure (2026-09-02, COMMIT EDİLMEDİ).** Kullanıcı isteği: tüm dating app'ler (Hinge/Raya referans ekran görüntüleri) tek uzun form yerine tek-soru-tek-ekran ilerliyor; eski `profile-setup/step1.tsx` (tek uzun scroll form) **9 ayrı ekrana** bölündü: `app/profile-setup/step1/{index(telefon),photos,verify(selfie),name,instagram,birthdate,location,gender,languages}.tsx`, ortak state `lib/onboardingStep1Context.tsx` (`Step1Provider`) + şablon `components/ui/QuestionScreen.tsx` (üstte geri-ok + ilerleme çubuğu, altta sabit "Next" butonu). Sıra: telefon → fotolar (6 slot, otomatik ilk-boş-slota-atama) → selfie doğrulama → ad-soyad → instagram (opsiyonel) → doğum tarihi (native wheel picker, `@react-native-community/datetimepicker`) → konum (şehir chip + ilçe text + **mesafe tercihi chip'leri**, `DISCOVERY_DISTANCE_OPTIONS`) → cinsiyet+buluşma tercihi → diller (son ekran, `submitAll()` tetikler). **Bilinçli renk kararı:** bu akışta app'in altın accent'i (`colors.accent`) TAMAMEN kaldırıldı, Raya'nın siyah/monokrom stiline geçildi (`#1A1A1A` — `Chip`'e yeni `selectedColor` prop'u, `QuestionScreen`'in buton/ilerleme-çubuğu rengi) — **sadece bu 9 ekranı kapsıyor, app'in geri kalanındaki altın kimliğe dokunulmadı** (`Chip`/`PrimaryButton` varsayılanları değişmedi). `register.tsx` sadeleştirildi (sadece email+şifre, telefon alanı kaldırıldı — `lib/countries.ts`'e taşındı). Yol boyunca cihazda bulunan/düzeltilen buglar: (1) `app/profile-setup/_layout.tsx` yoktu → Expo Router otomatik bir header'lı navigator sarıyordu, native "profile-setup/step1" başlığı klavyeyle çakışan Next butonunu gizliyordu — `headerShown:false` Stack ile düzeltildi. (2) telefon ekranında klavye Next butonunu kapatıyordu → `QuestionScreen`'e `KeyboardAvoidingView` eklendi. (3) foto slotlarına tıklayınca tam o slota değil, ilk boş slota otomatik atansın istendi. **tsc temiz, ama uçtan uca (baştan sona yeni bir hesapla) cihazda henüz test edilmedi — kullanıcı "testi sonra yaparım" dedi, bilinçli olarak commit edilmedi.** Bir sonraki oturumda: kullanıcı test ettiğinde commit et, sonra bu notu "Bitmiş"e taşı.
