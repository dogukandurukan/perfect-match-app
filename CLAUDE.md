...
**Açık:**
- 🔜 **SONRAKİ OTURUM — Alerts (`notifications.tsx`) → "Likes"-tarzı premium bölüm.** Hedef: alert ekranını Hinge/Tinder "Likes You" hissine yaklaştırmak, premium algısı (ör. beğenen sayısı vurgusu, blur/kilitli avatar + upgrade CTA, emoji tip-ikonları → gerçek vektör ikon `Ionicons`). **Kritik bağ:** gerçek "seni kim beğendi" verisi `likes` tablosunu gerektiriyor — o tablo HENÜZ YOK (§ çekirdek farklılaştırıcı). Yani: UI/premium kabuğu likes-backend'inden ÖNCE yapılabilir (mevcut `notifications` verisiyle), ama gerçek likes-akışı contextual `likes` işine bağlı. Başlarken önce ürün kararı: bu ekran salt-bildirim mi kalsın yoksa likes-merkezli mi olsun. Mevcut hal: `notifications` tablosu, emoji ikon (`typeIcon`), okundu/okunmadı, `resolveNotificationDisplayText`. rn-ui-reviewer + ui-ux-pro-max skill devreye alınabilir.
