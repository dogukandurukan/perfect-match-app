...
- 🔜 **Gün-içi buluşma hatırlatma bildirimi — büyük iş, ürün fikri (kullanıcı önerisi, 2026-08-21).** Fikir: buluşma günü "Bugün Derya ile buluşuyor musun?" bildirimi → Yes: "Harika, bugün buluşuyorsun!" / No: "Neden olmadı?" (opsiyonel yorum, `checkin.tsx`'in `wentThere`/rating akışına benzer bir öncesi). **İKİ blocker var, önce onlar çözülmeli:** (1) `confirmed_slot`/`confirmed_place` şu an serbest metin ("Friday evening 20?" gibi), gerçek tarih/saat DEĞİL — "bugün mü" kontrolü yapabilmek için slot sistemi gerçek `Date`'e geçmeli (mevcut `matches.meeting_at` timestamptz kolonu bunun için var ama hiç kullanılmıyor, tam bu iş için). (2) Zamanlanmış/push bildirim altyapısı yok (Expo Go modu, dev-build göçü ayrı büyük iş, §2'de not var). Bu iki blocker çözülmeden UI tasarımı anlamsız. Henüz başlanmadı.
