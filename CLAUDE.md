...
**Açık:**
- 🔜 **"Pick time" gerçek zaman+mekan seçme akışı yok** — hâlâ sadece chat'e deep-link. Yeni ekran/veri modeli gerekir (`matches`/yeni tablo — `lib/matchInvite.ts`'teki `IntroAnswers{place,slot1-3}` + `user_a_intro_answers`/`user_b_intro_answers` jsonb kolonları zaten kısmi altyapı olabilir, `micro-intro.tsx` ile bağlantılı — incelemeden varsayma). `supabase-expert` gerekir. Sıraya alındı, henüz başlanmadı.
- 🔜 **Buzz "activity history" feed — büyük iş, ürün fikri.** Instagram Activity ekranı referans: tarihe göre gruplanmış geçmiş (Bugün/Dün/Son 7 gün) — kime like/Note attın, hangi davete yes/no dedin vb. Bugünkü "demote to feed" küçük/geçici bir versiyonu (yukarıda) — asıl istenen çok daha kapsamlı, `likes`+`matches`+`notifications`'ı birleştiren ayrı bir liste-ekranı. Henüz başlanmadı.
- 🔜 **Buzz "activity history" feed — büyük iş, ürün fikri (2026-08-19, kullanıcı önerisi).** Şu an likes teaser tek başına, aşağısı bomboş — "tamamen boş hissettirmesin" isteği. Instagram Activity/Aktivite ekranı referans: tarihe göre gruplanmış geçmiş (Bugün/Dün/Son 7 gün) — kime like/Note attın, hangi davete yes/no dedin, vb. Veri kaynağı olası: `likes` (liker_id=ben olan satırlar) + `matches`/`notifications` (cevapladığın davetler). UI-3'ün (empty/0-state) ötesinde ayrı bir liste-ekranı işi; hangi olay tiplerinin "history" sayılacağına önce karar verilmeli. Henüz başlanmadı.
