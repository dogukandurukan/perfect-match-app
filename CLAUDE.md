...
- 🔜 **Blocker #2 — zamanlanmış/push bildirim altyapısı hâlâ yok** (Expo Go modu, dev-build göçü ayrı büyük iş, §2'de not var). Blocker #1 artık çözüldüğü için "bugün mü" SQL kontrolü (`meeting_at::date = current_date`) teknik olarak mümkün hâle geldi — ama zamanlanmış tetikleyici (cron/Edge Function) + push gönderme mekanizması hâlâ yok. Sıradaki mantıklı adım: dev-build (EAS) göçü, çünkü push bildirimi Expo Go'da çalışmıyor.
- 🔜 **Gün-içi buluşma hatırlatma bildirimi — orijinal ürün fikri (kullanıcı önerisi, 2026-08-21).** "Bugün Derya ile buluşuyor musun?" → Yes: "Harika, bugün buluşuyorsun!" / No: "Neden olmadı?" (opsiyonel yorum, `checkin.tsx`'in `wentThere`/rating akışına benzer bir öncesi). Blocker #1 bitti, Blocker #2 (push) kaldı — o olmadan bu özellik anlamsız.
