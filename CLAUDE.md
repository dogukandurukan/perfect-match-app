...
**Açık:**
- 🔜 **UI-4 — rn-ui-reviewer pass + push.** Bugünkü/dünkü redesign + Buzz read-side + UI-3 işi için bağımsız review; sonra push.
- 🔜 **Buzz featured card CTA'ları — büyük iş, ürün kararı gerekir (2026-08-18, kullanıcı önerisi).** Şu an "Respond" → Matches sekmesine genel yönlendirme, "Pick time" → o kişiyle chat'e deep-link (bugün düzeltildi, bkz commit `9509ad2`). Kullanıcı önerisi: **"Respond" inline Yes/No olmalı** (Matches'e gitmeden, kartın üzerinde hızlı kabul/red — `matches.tsx`'teki mevcut `acceptMatchInvite` fonksiyonu reuse edilebilir); **"Pick time" gerçek bir zaman+mekan seçme akışı olmalı** (şu an böyle bir ekran/veri modeli YOK — `matches`/yeni bir tabloya önerilen buluşma saati+yer eklenmeli, `supabase-expert` gerekir). Küçük routing fix'i (chat deep-link) bunun yerine geçici bir iyileştirme, asıl çözüm değil. Sıraya alındı, henüz başlanmadı.
