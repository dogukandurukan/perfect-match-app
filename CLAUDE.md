...
**Açık:**
- 🔜 **"Pick time" — kesin saat/mekan onayı hâlâ yok.** Kabul edilen bir davette 3 öneriden birini taraflardan biri (ya da her ikisi) kesinleştirsin diye küçük bir adım/ekran gerekiyor — `checkin.tsx`'e benzer bir "confirm" akışı olabilir. `supabase-expert` gerekmeyebilir (mevcut jsonb kolonlar yeterli olabilir, sadece yeni bir "confirmed_slot" alanı eklenir) — önce ürün kararı: kim kesinleştirir, tek taraf mı yeter mi.
