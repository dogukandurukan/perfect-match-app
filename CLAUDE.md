...
**Test kolaylığı — `components/ui/DevStepNav.tsx` (yeni, `__DEV__`-gated, üretimde hiç render olmuyor):** sağ üstte küçük **1 2 3 4** — hangisine dokunursan o step'e atlıyorsun (aktif altın renkte) + kırmızı **⎋** çıkış butonu (bayat/yenilenemeyen bir oturuma takılınca hızlı sign-out+login için). Kalıcı bir özellik değil, onboarding stabilize olunca kaldırılabilir.
