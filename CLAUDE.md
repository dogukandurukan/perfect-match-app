...
- 🔜 **Gün-içi buluşma hatırlatma bildirimi — orijinal ürün fikri (kullanıcı önerisi, 2026-08-21).** "Bugün Derya ile buluşuyor musun?" → Yes: "Harika, bugün buluşuyorsun!" / No: "Neden olmadı?" (opsiyonel yorum, `checkin.tsx`'in `wentThere`/rating akışına benzer bir öncesi). Her iki blocker da bitti (gerçek tarih + push altyapısı) — artık gerçekten yapılabilir. Zamanlanmış tetikleyici (cron/pg_cron ya da Edge Function + Supabase Scheduled Trigger) tasarlanmalı, `meeting_at::date = current_date` filtresiyle. Henüz başlanmadı.
