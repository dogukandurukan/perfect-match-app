...
| gender | `profiles.gender` (Man/Woman/Non-binary) | `profiles.gender_v2` — ✅ D15 Woman/Man/Non-binary, single; 🔵 keys `woman/man/non_binary` (draft's `self_describe` + `gender_self_describe` to be removed) | required | public | REVISE |
| interested_in | `profiles.meeting_preferences` (Men/Women/Non-binary/Everyone) | `discovery_preferences_v2.interested_in_v2 text[]` — ✅ D15b; 🔵 keys `woman/man/non_binary` or `everyone` alone (draft's exclusive-`everyone` CHECK kept; `self_describe` removed); min 1 | required | private | REVISE |
