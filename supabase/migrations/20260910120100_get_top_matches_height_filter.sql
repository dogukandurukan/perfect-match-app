-- Wires discovery_height_min/max into candidate filtering — height itself
-- is NOT part of raw_score (deliberately not scored, filter-only). NULL on
-- either side (my preference unset, or candidate never entered their
-- height) is permissive — doesn't exclude. Byte-identical otherwise to the
-- previous version (20260910110100, advanced-filters WHERE additions).
...
candidates AS (
  ...
  WHERE p.id != p_user_id
    ...
    -- Height filter (2026-09-10) — NOT scored, WHERE-only. NULL on my
    -- preference or on the candidate's own height is permissive.
