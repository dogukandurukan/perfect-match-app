-- get_top_matches: hard cap at 85% when age is a penalty (2026-09-15)
-- User found a live, real example: 10-year age gap (41 vs 31) scored 99%
-- because every other lifestyle category (intent, hobbies, availability,
-- drinking, smoking, education, environment) happened to align almost
-- perfectly. Verified by hand: 138/140 raw score, exactly what the
-- deployed formula produces — not a bug, a structural imbalance. The age
-- penalty tops out at -5, but the "lifestyle" categories together carry
-- ~135 points of budget, so a -5 penalty can't meaningfully pull down a
-- near-maxed total.
--
-- Rather than reshuffling weights/normalizer again (affects everyone,
-- imprecise), a targeted fix: when age_score is negative (7+ year gap),
-- the displayed % can never exceed 85 — "Great match" ceiling, never
-- "Exceptional/near-perfect" — regardless of how well everything else
-- lines up. match_category is derived from the SAME capped value so the
-- label and number never disagree. Nothing else about the formula
-- changed.
...
capped AS (
  -- Age-gap ceiling (2026-09-15): when age_score is negative (7+ year
  -- gap), the % can never read as "basically perfect" no matter how well
  -- everything else lines up — capped at 85 instead of the usual 99.
  SELECT r.*,
    LEAST(
