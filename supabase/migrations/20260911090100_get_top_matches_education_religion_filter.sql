-- Wires discovery_education/discovery_religion into candidate filtering —
-- NOT scored (education_score stays exactly as-is, separate mechanism),
-- filter-only. NULL on my preference, an empty array, OR the candidate's
-- own value being NULL is permissive — doesn't exclude. Byte-identical
-- otherwise to the previous version (20260910130100, zodiac/pets filter).
...
WITH me AS (
  SELECT p.id, p.gender, p.meeting_preferences, p.city, p.district, p.date_of_birth,
    ...
    p.discovery_education, p.discovery_religion,
    oa.intent
  FROM profiles p
...
candidates AS (
  ...
  WHERE p.id != p_user_id
    ...
    -- Education / religion filters (2026-09-11) — NOT scored, WHERE-only.
    AND (
