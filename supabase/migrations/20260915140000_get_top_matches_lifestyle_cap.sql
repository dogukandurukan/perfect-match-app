-- get_top_matches: cap the "lifestyle" category bucket at 65 (2026-09-15)
-- Structural fix, not another one-off patch. Found via the Aylin question
-- ("why 99% with no age gap involved") that the real recurring problem
-- isn't age specifically — it's that 9-11 independent "lifestyle" signals
-- (morning/night, recharge, hobbies, availability, drinking, smoking,
-- education, language, environment, zodiac, favorite spot) sum freely up
-- to ~130 points, and small enumerated option pools (8 hobby presets, a
-- handful of drinking/smoking/education values) make it statistically
-- easy for two people to agree on most of them by chance. When that
-- happens, no single "core" signal (intent/location/age) can pull the
-- score back down — that's what the age-only cap from migration
-- 20260915130000 didn't fully address (Aylin had no age penalty at all
-- and still hit 92-99%).
--
-- Fix: split raw_score into `core` (location + age + intent — always
-- fully additive, these are the signals that actually matter for "can we
-- meet and do we want the same thing") and `lifestyle_bucket` (everything
-- else). lifestyle_bucket is capped at 65 (roughly half its ~130-point
-- ceiling) before being added to core. Verified against 4 real
-- candidates before deploying (Buse 84->63%, Pelin 85->77%, Asli 99->88%,
-- Aylin 92->92% UNCHANGED because her lifestyle_bucket was already under
-- 65 — this cap only corrects genuinely inflated cases, not all high
-- scores). Normalizer stays 140, age-negative cap stays 85. No individual
