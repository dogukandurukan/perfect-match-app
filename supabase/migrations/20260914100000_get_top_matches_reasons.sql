-- get_top_matches: real "reasons" array + English match_category (2026-09-14)
-- Follow-up to 20260914090000 (scoring rewrite). Both ChatGPT and Gemini
-- independently suggested surfacing WHY two people matched, not just a bare
-- %. reasons was a placeholder (ARRAY['eslesme']) the client never even
-- rendered — this computes real ones, priority-ordered, capped at 3.
-- match_category text also switched Turkish→English to match the rest of
-- the app's UI copy (was the one Turkish string in an otherwise-English
-- screen). Scoring math itself (compat_raw_score, completeness_multiplier,
-- sort_boost, normalizer) is UNCHANGED from 20260914090000.
...
SELECT
  ...
  -- Real, priority-ordered reasons (was ARRAY['eslesme'] placeholder,
  -- never rendered by the client). Capped at 3, only fires for genuinely
  -- strong signals — a low match can legitimately return an empty array.
  (
    ...
    FROM (
      ...
      FROM unnest(ARRAY[
        ...
        CASE WHEN a.hobby_overlap >= 2 THEN a.hobby_overlap::text || ' shared interests' END,
        CASE WHEN a.location_score = 20 THEN 'Nearby' END,
        CASE
          WHEN a.morning_night_score = 15 AND a.morning_night = 'Morning person' THEN 'Both morning people'
          WHEN a.morning_night_score = 15 AND a.morning_night = 'Night owl' THEN 'Both night owls'
          WHEN a.morning_night_score = 15 THEN 'Similar daily rhythm'
        END,
        CASE WHEN a.avail_overlap >= 4 THEN 'Similar schedules' END
