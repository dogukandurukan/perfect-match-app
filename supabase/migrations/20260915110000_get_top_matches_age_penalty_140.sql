-- get_top_matches: age 7+ years -> -5 penalty, normalizer 150 -> 140 (2026-09-15)
-- User's explicit call after reviewing the 98%/81% Buse example: age
-- neutral-at-7+ wasn't wrong per se, but a small penalty at that range is
-- wanted rather than pure 0. Combined with lowering the normalizer to 140
-- (which on its own would have pushed percentages UP, the opposite of
-- what was wanted — flagged this directly before implementing).
...
candidates AS (
  SELECT p.id, p.first_name, p.date_of_birth, p.city, p.district, p.zodiac_sign, p.photos,
    ...
    -- 7+ years now -5 (was 0) — user's explicit call, 2026-09-15.
    CASE
...
SELECT
  ...
  LEAST(99, GREATEST(20, ROUND((a.adjusted_score / 140) * 100)))::integer AS match_percentage,
  CASE
    WHEN (a.adjusted_score / 140) * 100 >= 80 THEN '🔥 Exceptional match'
    WHEN (a.adjusted_score / 140) * 100 >= 60 THEN '✨ Great match'
    WHEN (a.adjusted_score / 140) * 100 >= 40 THEN '👍 Good match'
    ELSE '🤝 Possible match'
