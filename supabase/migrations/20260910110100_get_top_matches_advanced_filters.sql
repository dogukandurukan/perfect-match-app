-- Wires the two new Advanced-filter toggles (discovery_verified_only,
-- discovery_nonsmokers_only) into actual candidate filtering — a toggle
-- that doesn't change results isn't really a filter. Byte-identical
-- otherwise to the previous version (20260909100000, liked_me bonus).
...
candidates AS (
  ...
  WHERE p.id != p_user_id
    ...
    -- Advanced filters (2026-09-10) — NOT me.flag short-circuits to true
    -- when the toggle is off, so this is a no-op for the vast majority of
    -- users who never touch them.
