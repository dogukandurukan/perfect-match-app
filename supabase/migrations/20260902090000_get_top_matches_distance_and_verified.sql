...
scored AS (
  SELECT p.id, p.first_name, p.date_of_birth, p.city, p.district, p.zodiac_sign, p.photos,
    ...
    (
      ...
      + CASE
          ...
          ELSE 0
        END
      + CASE WHEN p.morning_night IS NOT NULL AND p.morning_night = me.morning_night THEN 15 ELSE 0 END
      + CASE WHEN p.recharge_style IS NOT NULL AND p.recharge_style = me.recharge_style THEN 15 ELSE 0 END
      + LEAST(25, (SELECT COUNT(*) * 5 FROM unnest(p.hobbies) h WHERE h = ANY(me.hobbies))::int)
      + CASE
          WHEN (SELECT COUNT(*) FROM unnest(p.availability_days) d WHERE d = ANY(me.availability_days)) >= 4 THEN 15
          WHEN (SELECT COUNT(*) FROM unnest(p.availability_days) d WHERE d = ANY(me.availability_days)) >= 2 THEN 8
          WHEN (SELECT COUNT(*) FROM unnest(p.availability_days) d WHERE d = ANY(me.availability_days)) >= 1 THEN 3
          ELSE 0
        END
      + CASE
      ...
      + CASE
          ...
          ELSE 0
        END
      + CASE WHEN p.education IS NOT NULL AND p.education = me.education THEN 8 ELSE 0 END
      + CASE WHEN (SELECT COUNT(*) FROM unnest(p.languages) l WHERE l = ANY(me.languages)) >= 1 THEN 5 ELSE 0 END
      + CASE
          WHEN (SELECT COUNT(*) FROM unnest(p.meeting_environment) e WHERE e = ANY(me.meeting_environment)) >= 2 THEN 12
          WHEN (SELECT COUNT(*) FROM unnest(p.meeting_environment) e WHERE e = ANY(me.meeting_environment)) >= 1 THEN 6
          ELSE 0
        END
      + CASE WHEN p.photo_verified = true THEN 15 ELSE 0 END
    ) AS raw_score
  FROM profiles p
  LEFT JOIN onboarding_answers oa ON oa.user_id = p.id
  CROSS JOIN me
  WHERE p.id != p_user_id
    ...
    AND (
      -- discovery_max_distance: 'whole_city' (or unset — every existing
      -- profile before this feature) keeps the original behaviour (city
      -- match only). 'same_neighborhood'/'same_district' both narrow to a
      -- district match — the schema only has one district-level field
      -- (profiles.district), there's no finer neighbourhood/district split
      -- to distinguish the two tiers by (2026-09-02).
