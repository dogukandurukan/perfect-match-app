-- get_top_matches: normalizer 125 -> 150 (2026-09-15)
-- Found live, testing on a real account: near-identical structural
-- overlap (hobbies, availability, recharge style, drinking, smoking,
-- education, language) on several candidates pushed compat_raw_score to
-- ~120+ out of 125, landing "Exceptional match" (90%+) on 4 of 5 cards —
-- including a 7-year age gap pair (122/125 = 98%). Root cause: 125 was
-- calibrated against a different viewer's candidate pool (via ChatGPT/
-- Gemini review) that didn't happen to cluster this tightly on the
-- lifestyle-preset fields. Age contributed only ~8 of those points (the
-- neutral-not-negative policy stays — both AI reviews and the Hinge
-- engagement data support it, and it wasn't the dominant driver here).
-- Reverting the divisor to 150 (the pre-2026-09-14 value) restores
-- headroom without touching any individual component weight — everything
-- else from today (sort_boost split, real reasons, zodiac/spot bonuses,
-- completeness multiplier) stays exactly as is.
...
SELECT
  ...
  LEAST(99, GREATEST(20, ROUND((a.adjusted_score / 150) * 100)))::integer AS match_percentage,
  CASE
    WHEN (a.adjusted_score / 150) * 100 >= 80 THEN '🔥 Exceptional match'
    WHEN (a.adjusted_score / 150) * 100 >= 60 THEN '✨ Great match'
    WHEN (a.adjusted_score / 150) * 100 >= 40 THEN '👍 Good match'
    ELSE '🤝 Possible match'
  ...
  (
    ...
    FROM (
      ...
      FROM unnest(ARRAY[
        ...
        CASE WHEN a.zodiac_score = 5 THEN 'Great zodiac match' END
