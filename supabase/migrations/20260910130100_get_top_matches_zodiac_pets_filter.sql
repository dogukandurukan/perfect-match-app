-- Wires discovery_zodiac_signs/discovery_pets into candidate filtering —
-- neither scored, filter-only (same as height). NULL on my preference,
-- an empty array, OR the candidate's own value being NULL (nobody has
-- set pets yet) is permissive — doesn't exclude. Byte-identical
-- otherwise to the previous version (20260910120100, height filter).
...
candidates AS (
  ...
  WHERE p.id != p_user_id
    ...
    -- Zodiac / pets filters (2026-09-10) — NOT scored, WHERE-only.
    AND (
