-- get_top_matches: zodiac compatibility bonus + richer reasons (2026-09-14)
-- User feedback on the previous reasons list: "Both morning people" /
-- "Similar schedules" felt weak. Requested instead: drinking/smoking match,
-- meeting-environment overlap, named shared hobbies, and zodiac
-- compatibility (explicitly: "not scientific, but a lot of people believe
-- in it and it's a fun signal").
--
-- Zodiac scoring (new, never negative — same "don't punish, only reward"
-- philosophy as the rest of the formula):
--   +10  candidate is in the viewer's "top 3" — the 2 same-element
--        signmates + the traditional opposite/polarity sign (Aries<->Libra,
--        Taurus<->Scorpio, Gemini<->Sagittarius, Cancer<->Capricorn,
--        Leo<->Aquarius, Virgo<->Pisces). Symmetric by construction.
--   +5   general element-group compatibility (Fire+Air or Earth+Water)
--        that isn't already a top-3 pick.
--    0   anything else, or either sign missing.
-- Small relative to the rest of the formula (max +10, same scale as
-- education's +8) — a fun signal, not a dominant one.
