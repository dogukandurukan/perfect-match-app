...
-- Overlap counts computed once each (previously the availability_days count
-- was re-run as 3 separate correlated subqueries — one per >=4/>=2/>=1
-- threshold — and meeting_environment's as 2; same story for a MATERIALIZED
-- boundary below). Perf-only change, scoring output is unchanged
-- (2026-09-08, found via EXPLAIN ANALYZE: planning time alone was 187ms on
-- an inlined SQL function whose raw_score expression got re-evaluated 3x
-- downstream — in match_percentage, match_category, and ORDER BY).
candidates AS (
