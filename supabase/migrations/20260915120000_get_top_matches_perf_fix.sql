-- get_top_matches: defer expensive `reasons` computation until after
-- ranking + LIMIT (2026-09-15, perf regression found via systematic
-- debugging — user reported Matches/Chat/Activity transitions still slow
-- after the auth getUser->getSession fix helped Home).
--
-- Measured: 125ms execution (EXPLAIN ANALYZE), vs ~9ms after the
-- 2026-09-08 optimization pass. Root cause: candidates for a typical
-- viewer number ~349 rows (same-city pool before per-user discovery
-- filters), and the `reasons` array — a correlated subquery doing
-- unnest+WITH ORDINALITY+ORDER BY+LIMIT per row — was being evaluated for
-- ALL 349 candidates in the final SELECT, even though only p_limit (10)
-- rows are ever returned. Same for the zodiac cross-join EXISTS check
-- (up to 144 combinations) inside `candidates` — that one is harder to
-- defer since it feeds the ranking score itself, left as-is for now.
--
-- Fix: rank + LIMIT in a new `ranked` CTE first, then compute
-- match_percentage/match_category/reasons only on those final rows.
-- Nothing about the scoring formula changed — same weights, same
-- normalizer (140), same age penalty, same everything from the last
-- three migrations. Pure query-shape optimization.
...
-- Rank + LIMIT BEFORE computing the expensive reasons array — that used
-- to run once per candidate (349 for a typical viewer) instead of once
-- per returned row (p_limit, usually 10). This is the actual perf fix.
ranked AS (
  SELECT a.*
  FROM adjusted a
...
SELECT
  r.id AS user_id, r.first_name, r.date_of_birth, r.city, r.district, r.zodiac_sign, r.photos,
  LEAST(99, GREATEST(20, ROUND((r.adjusted_score / 140) * 100)))::integer AS match_percentage,
  CASE
    WHEN (r.adjusted_score / 140) * 100 >= 80 THEN '🔥 Exceptional match'
    WHEN (r.adjusted_score / 140) * 100 >= 60 THEN '✨ Great match'
    WHEN (r.adjusted_score / 140) * 100 >= 40 THEN '👍 Good match'
    ELSE '🤝 Possible match'
  ...
  (
    ...
    FROM (
      SELECT reason, ord
      FROM unnest(ARRAY[
        CASE WHEN r.intent_score = 40 THEN 'Looking for the same thing' END,
        CASE WHEN array_length(r.shared_hobbies,1) >= 1
          THEN 'You both love ' || array_to_string(r.shared_hobbies[1:2], ' & ') END,
        CASE WHEN r.location_score = 20 THEN 'Nearby' END,
        CASE WHEN r.spot_score = 5 THEN 'Same favorite spot' END,
        CASE WHEN r.drinking_score = 10 THEN 'Similar drinking habits' END,
        CASE WHEN r.smoking_score = 10 THEN 'Similar smoking habits' END,
        CASE WHEN r.env_overlap >= 1 THEN 'Same idea of a first date' END,
        CASE WHEN r.zodiac_score = 5 THEN 'Great zodiac match' END
  ...
  r.favorite_music, r.favorite_movie, r.favorite_book, r.hobbies, r.availability_days,
  r.drinking, r.smoking, r.education, r.education_detail, r.morning_night
FROM ranked r
ORDER BY (r.adjusted_score + r.sort_boost) DESC;
$function$;
