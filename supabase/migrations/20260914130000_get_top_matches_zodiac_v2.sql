-- get_top_matches: zodiac weight lowered + "Nearby" reordered (2026-09-14)
-- Final tuning pass after sharing the algorithm with ChatGPT + Gemini for
-- a last sanity check before freezing V1.
--
-- 1. Zodiac top-3 bonus +10 → +5. Both reviews approved the system
--    overall; ChatGPT specifically flagged that a signal we ourselves
--    documented as having no scientific basis was sitting at or above
--    genuinely evidence-based signals (education +8, drinking/smoking
--    +10, language/favorite-spot +5) — an inconsistent hierarchy. +5 puts
--    it alongside the other "small fun/behavioral" bonuses instead of
--    competing with structural ones. General element-group tier scaled
--    down proportionally, 5 → 2, to keep the two tiers distinguishable.
-- 2. "Nearby" moved from priority #6 to #3 in the reasons list — Gemini's
--    point: same-district is a strong "we can actually meet easily"
--    signal, capped at 3 shown reasons meant it rarely surfaced.
-- Nothing else changed from 20260914120000.
